...
## `#generated-in-their-tree` — a file we generate lives under somebody else's tooling

**2026-09-05, measured in the field eight minutes after an install.** `HartEdge-admin` ran `prettier
--write .` and it rewrote `.claude/comm-hook.mjs`: 352 lines, double quotes to single, semicolons added,
`catch { return false }` expanded. The stub's first line reads `GENERATED by claude-comm install.mjs — do
not edit`. **A formatter does not read English**, and neither does a linter, a codemod, or an editor's
format-on-save.

Two consequences, and the second is the one that matters because it is THEIR cost, not ours:

1. `install --check` reports that agent out of date forever — reinstall, reformat, repeat.
2. Their `lint` script is `prettier --check .`, so **a file this installer puts in their tree fails their
   lint**. Invisible until somebody runs `format` on that project, which is why it took a day to surface.

The fix is the `.gitignore` rule's twin, and its LIMIT is the design: the installer adds the two paths it
writes to `.prettierignore` **only where a formatter is already configured**. A project with no prettier
config gets nothing — creating formatter configuration in somebody's repository because we have an opinion
is not an installer's business, and `A43`'s control is exactly that project.

**The general shape, worth carrying beyond prettier:** anything this tool writes into a project it does not
own will eventually be picked up by that project's own automation. `.gitignore` was the first instance and
was treated as a one-off; this is the second, in a different tool, and the pattern is now named. The next
candidates are ESLint (a generated file failing `eslint .`), and any repo-wide codemod.
