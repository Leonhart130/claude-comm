 ...
 *
 * Everything else is `bin/wake.mjs`'s rules, reused rather than reimplemented — resolve by
 * pid then send, never `--match` on a guess (it exits 0 on no match); every socket, not just
 * `$KITTY_LISTEN_ON`; no daemon; a quiet period, which is a written record consulted by the
 * next run, not a timer; and — only since 2026-09-13, A64 — **never into a running turn**.
 * This paragraph said "five rules, unchanged and reused" for a whole release after `wake`
 * learned the sixth, and the bell went on typing into peers' turns. FINDINGS.md#bell-mid-turn
 *
...
/** A refusal: usage exit 64 with a reason, THROWN, so the function returns it instead of exiting its caller. */
class Refusal extends Error {}

/**
 * THE WHOLE BELL AS A FUNCTION, so a gate can EXECUTE it (A64). Until 2026-09-13 this was a script with
 * `process.exit` on every path, and the only way to reach its ring was a peer leader alive under kitty — which no
 * machine running a gate has. So rule 6 could only have been gated by reading this source, the shape review #6 F4
 * caught in A35. `wins`, `lookup`, `send` and `who` are the four things a test machine lacks; every other line is
 * the code the CLI runs. Returns { code, out, err }: 0 rang or would ring · 3 not rung (not running, mid-turn,
 * quiet period) · 64 refused.
 */
export async function bell(argv, { wins = null, lookup, send = null, who = null } = {}) {
	...
	try {
		...
		// Two statements, not one try: with a THROWING die, "is not a file" raised inside the try became "cannot be read".
		let st = null
		try { st = statSync(ref) } catch { die(`--ref cannot be read: ${ref}`) }
		if (!st.isFile()) die(`--ref is not a file: ${ref}`)
		...
		// inferring it.
		if (has("--print-text")) return { code: 0, out: TEXT + "\n", err: "" }
		...
		if (!pids.length) {
			...
			return { code: 3, out: `${agent} is not running in ${project} — nothing rung. The file is written and waits: ${ref}\n`, err: "" }
		}

		const wake = await import(pathToFileURL(join(ROOT, "bin", "wake.mjs")).href)
		if (!wins) wins = wake.windows()
		...
		// RULE 6, wake.mjs's — the one this file claimed to reuse and did not. `wake` stopped typing into running turns in
		// `.5` (A62); this bell went on, so a letter rung while a peer leader works lands INSIDE that turn, in the window
		// its owner talks in. Busy is neither rung nor recorded, so the next run looks again; unknown rings as before and
		// says so; a reply that just ended rings - all exactly as `wake` decides. A64, FINDINGS.md#bell-mid-turn
		if (lookup === undefined) {
			try { ({ lookup } = await import(pathToFileURL(join(ROOT, "bin", "session-registry.mjs")).href)) } catch { lookup = null }
		}
		const turn = wake.readTurn(target.pid, lookup)
		if (turn.state === "busy") return { code: 3, out: `○ ${agent} (pid ${target.pid}) is mid-turn — not rung (${turn.why}). The file is written and waits: ${ref}\n`, err: "" }
		const turnNote = turn.state === "unknown" ? ` · turn state unknown (${turn.why}), rung as before` : turn.state === "ending" ? " · its reply had just ended" : ""
		...
		if (sinceMs < QUIET_MS && !has("--force")) {
			return { code: 3, out: `rung ${Math.round(sinceMs / 1000)}s ago (quiet period ${QUIET_MS / 1000}s) — not ringing. --force overrides.\n`, err: "" }
		}

		if (has("--dry-run")) return { code: 0, out: `would ring ${agent} (pid ${target.pid}) in window ${target.r.win.id} on ${target.r.win.sock} — ${target.r.how}${turnNote}\n  ${TEXT}\n`, err: "" }
		const typeInto = send ? (t) => send(target.r.win, t) : (t) => spawnSync("kitten", ["@", "--to", `unix:${target.r.win.sock}`, "send-text", "--match", `id:${target.r.win.id}`, t],
			{ encoding: "utf8", timeout: 5000 })
		const a = typeInto(TEXT)
		...
		return { code: 0, out: `● rang ${agent} (pid ${target.pid}) in window ${target.r.win.id} — ${target.r.how}${turnNote}\n  ref: ${ref}\n`, err: "" }
	} catch (e) {
		if (e instanceof Refusal) return { code: 64, out: "", err: `exchange-bell: ${e.message}\n` }
		throw e
	}
}

// Run only when this file IS the program — the guard wake.mjs and session-registry.mjs use — so a gate can import it.
if (process.argv[1] && process.argv[1] === fileURLToPath(import.meta.url)) {
	const r = await bell(process.argv.slice(2))
