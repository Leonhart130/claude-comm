...
const SKILL = (id, agentRoot) => {
	...
	const experts = Object.keys(cfg.agents).filter((a) => a !== leader)
	const L = []
	L.push("---", "name: claude-comm",
		...
		isLeader
			? `You are **\`${id}\`, the leader** — ${experts.length ? `experts: ${experts.map((e) => "`" + e + "`").join(", ")}` : "no expert on the roster yet"}. Every command below runs from your own folder.`
